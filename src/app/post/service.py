...
from .models import Post, Hashtag,post_hastags
...
async def create_hastags_svc(db: Session, post: Post):
    regex =r"#\w+"
    ...
    for match in matches:
        ...
        hashtag = db.querry(Hashtag).filter(Hashtag.name == name).first()
        if not hashtag:
...
async def create_post_svc(db:Session, post:PostCreate, user_id: int):
    ...
    await create_hastags_svc(db, db_post)
...
async def get_posts_from_hashtag_svc(db:Session, hashtag_name: str):
    hashtag= db.querry(Hashtag).filter_by(name=hashtag_name).first()
    if not hashtag:
        return None
...
async def get_random_posts_svc(db:Session, page: int=1, limit: int = 10, hashtag: str = None):
    ...
    if hashtag:
        posts = posts.join(post_hastags).join(Hashtag).filter(Hashtag.name == hashtag)
...
async def unlike_post_svc(db:Session, post_id:int, username:str):
    ...
    if not user in post.liked_by_users:
        return False, "user not liked the post" 
...
async def liked_by_users_post_svc(db:Session, post_id: int) -> list[UserSchema]:
    post = await get_post_from_post_id_svc(db, post_id)
    if not post:
        return[]
